...
function addClick(e){
	...
	calculate(obj.pLen * obj.pAmount,function(){
		if (conditionBoo){
			if(exist(obj.pID) == "no"){
				cartAry.push(obj);
			}else{
...
function delClick(e){
	...
	var pAmount = target.find('.priceBox span').text();

	calculate(-selectNum * pAmount ,function(){
...
function selectChange(e){
	...
	calculate((lastChooseNum - currentChooseNum)  * pAmount, function(){  //原數值和新數值的差值
		if (conditionBoo) {
...
function calculate(num,callback){
	...
	for (var i = 0; i < cartAry.length; i++) cntTotalPrice += cartAry[i].pLen * cartAry[i].pAmount ;

    // conditionBoo = !Boolean(cntTotalPrice + num > require);
	if (conditionBoo) {
        $(".promotions-bar .orange-btn").attr('disabled', Boolean(cntTotalPrice + num < require));
	}else{
		// alert("超過可選最大數");
	}
...
function loadCartData(){
	...
	$(".require-box .require").text(require);
